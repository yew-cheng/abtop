...
```bash
abtop                    # Launch TUI + background HTTP server (port 8787)
abtop --tui              # Launch TUI only
...
abtop --http 8080        # Run headless HTTP server on a custom port
```
...
By default the TUI also starts a background HTTP server on port 8787. To run
headlessly without the TUI:
